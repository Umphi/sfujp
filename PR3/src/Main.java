...
/**
 * Главный класс приложения для управления информацией о животных.
 * Предоставляет интерфейс командной строки.
 */
public class Main {
  /**
   * Функция для парсинга int из строки с проверкой.
   *
   * @param str Строка из которой нужно парсить значение
   * @param result Куда нужно записать результат
   * @return true если парсинг удался, false - если не удался
   */
    ...
    while (true) { 
      ...
      switch (input) {
        case "1":
          ...
          switch (chosen[0]) {
            case 1:
              ...
              while (!tryParseInt(inputParameter, chosenYear)
                    || chosenYear[0] < 500
                    || chosenYear[0] > LocalDate.now().getYear()
              ) {
              ...
              while (!tryParseInt(inputParameter, averageHeartRate)
                    || averageHeartRate[0] < 1
                    || averageHeartRate[0] > 1000
              ) {
            ...
            case 2:
              ...
              biteForce[0] = -1;
              while (!tryParseInt(inputParameter, biteForce)
                  || biteForce[0] < 0
                  || biteForce[0] > 10000
              ) {
            ...
            case 3:
              ...
              finCount[0] = -1;
              while (!tryParseInt(inputParameter, finCount)
                  || finCount[0] < 0
                  || finCount[0] > 30
              ) {
              ...
              while (!tryParseInt(inputParameter, waterType)
                  || waterType[0] < 1
                  || waterType[0] > 3
              ) {
            ...
            case 4:
              ...
              while (!tryParseInt(inputParameter, bodyTemperature)
                  || bodyTemperature[0] < 1
                  || bodyTemperature[0] > 100
              ) {
        ...
        case "2":
          ...
          removeIndex[0] = -1;
          while ((
                !tryParseInt(removeIndexInput, removeIndex)
                || removeIndex[0] < 0 
                || removeIndex[0] > animals.size() - 1
              )
              && !removeIndexInput.equals("c")
          ) {
        ...
        case "4":
          ...
          checkIndexFirst[0] = -1;
          while ((
              !tryParseInt(checkIndexInput, checkIndexFirst)
              || checkIndexFirst[0] < 0
              || checkIndexFirst[0] > animals.size() - 1
            ) 
            && !checkIndexInput.equals("c")
          ) {
          ...
          while ((
              !tryParseInt(checkIndexInput, checkIndexSecond)
              || checkIndexSecond[0] < 0
              || checkIndexSecond[0] > animals.size() - 1
            )
            && !checkIndexInput.equals("c")
          ) {
